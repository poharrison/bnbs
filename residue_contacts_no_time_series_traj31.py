...
class Trajectory():
    def __init__(self, traj_num, trajectories, nres1, nres2):
        ...
        self.atom_contacts = np.full((652,878,718), 0)
        self.contacts = np.full((652, nres1, nres2), 0)
        self.summed_contacts = np.empty(shape=(nres1, nres2), dtype=np.int32)
...
def main():
    ...
    traj31.sum_contacts()
    traj31.flatten_contacts()
    print(traj31.flattened_contacts)
    f = open("/home/poh8/bnbs_storage/analysis/combined_analysis/traj31_flat.pickle", "wb")
    pickle.dump(traj31.flattened_contacts, f)
